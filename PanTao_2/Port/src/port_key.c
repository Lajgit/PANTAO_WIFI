...
void Key_Init(Key_HandleTypeDef *key, uint16_t ID, GPIO_TypeDef *GPIO_Port, uint16_t GPIO_Pin,
              uint32_t debounce_time, uint32_t longpress_time, uint32_t trigger_frequnecy,
              void (*short_callback)(uint16_t), void (*long_callback)(uint16_t), void (*release_callback)(uint16_t), GPIO_PinState trigger_level)
{
    if (key == NULL)
        return; // 检查指针有效性
    ...
    key->port = GPIO_Port;
    key->pin = GPIO_Pin;
    key->last_state = !trigger_level;
    key->current_state = key->last_state;
    key->debounce_time = debounce_time;
    key->long_press_time = longpress_time;
    key->press_tick = Get_Systime();
    key->release_tick = Get_Systime();
    key->hold_time = 0;
    key->key_state = KEY_STATE_IDLE;
    key->trigger_count = Get_Systime();
    key->trigger_frequnecy = trigger_frequnecy;
    key->short_callback = short_callback;
    key->long_callback = long_callback;
    key->release_callback = release_callback;
    key->trigger_level = trigger_level;
}
